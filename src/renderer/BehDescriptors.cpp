...
bool DescriptorAllocator::allocate(vk::DescriptorSet *set, vk::DescriptorSetLayout layout,
	...
	vk::DescriptorSetVariableDescriptorCountAllocateInfo setCounts = {.descriptorSetCount = 1,
...
vk::DescriptorSetLayout DescriptorLayoutCache::createDescriptorLayout(vk::DescriptorSetLayoutCreateInfo *info) {
	...
	if (!isSorted) {
		std::sort(
			...
			[](vk::DescriptorSetLayoutBinding &a, vk::DescriptorSetLayoutBinding &b) { return a.binding < b.binding; });
	}
...
DescriptorSetLayoutBuilder &DescriptorSetLayoutBuilder::addBinding(uint32_t binding, vk::DescriptorType type,
																   vk::ShaderStageFlags stageFlags, uint32_t count,
																   vk::DescriptorBindingFlags flags) {
	vk::DescriptorSetLayoutBinding newBinding{
		.binding = binding, .descriptorType = type, .descriptorCount = count, .stageFlags = stageFlags};
	...
	for (size_t i = 0; i < bindingFlags.size(); i++) {
		...
		if (bindingFlag & vk::DescriptorBindingFlagBits::eVariableDescriptorCount && i + 1 != bindingFlags.size()) {
			std::cerr << "VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT may only be set on the last descriptor, "
						 "but is set on binding["
					  << i << "]. VUID-VkDescriptorSetLayoutBindingFlagsCreateInfo-pBindingFlags-03004\n";
			result = false;
		}
	}
...
DescriptorSetBuilder &DescriptorSetBuilder::bindBuffer(uint32_t binding, vk::DescriptorBufferInfo *bufferInfo,
...
DescriptorSetBuilder &DescriptorSetBuilder::bindImage(uint32_t binding, vk::DescriptorImageInfo *imageInfo,
													  vk::DescriptorType type) {
	vk::WriteDescriptorSet newWrite{
		.dstBinding = binding,
		.descriptorCount = 1,
...
DescriptorSetBuilder &DescriptorSetBuilder::bindImages(uint32_t binding,
													   std::vector<vk::DescriptorImageInfo> &imageInfos,
													   vk::DescriptorType type) {
	vk::WriteDescriptorSet newWrite{.dstBinding = binding,
