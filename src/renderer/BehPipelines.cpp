...
BehPipeline::BehPipeline(std::shared_ptr<BehDevice>& device, PipelineConfigurationInfo& config) {
	...
	for (const auto& [filepath, stage] : config.shaders) {
		...
		vk::PipelineShaderStageCreateInfo shaderStageCreateInfo{
			.stage = stage, .module = shaderModules.back(), .pName = "main"};
		shaderStageCreateInfos.push_back(shaderStageCreateInfo);
		isComputePipeline |= stage == vk::ShaderStageFlagBits::eCompute;
	}

	if (isComputePipeline) {
		vk::ComputePipelineCreateInfo pipelineInfo{.stage = shaderStageCreateInfos[0], .layout = config.pipelineLayout};
...
void BehPipeline::createGraphicsPipeline(PipelineConfigurationInfo& config,
										 std::vector<vk::PipelineShaderStageCreateInfo>& shaderStageCreateInfos) {
...
vk::ShaderModule BehPipeline::createShaderModule(const std::vector<char>& code) {
	const vk::ShaderModuleCreateInfo createInfo{.codeSize = code.size(),
