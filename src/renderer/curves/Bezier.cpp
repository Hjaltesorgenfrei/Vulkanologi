...
Bezier::Bezier(std::vector<std::shared_ptr<ControlPoint>> controlPoints, glm::vec3 color)
...
std::vector<FrenetFrame> Bezier::getRMFrames(Point a, glm::vec3 b, glm::vec3 c, glm::vec3 d,
	...
	auto x0 = FrenetFrame{.o = a.position, .t = tangentCubicCurve(a.position, b, c, d, evenTs[0]), .n = a.normal};
	...
	for (int i = 1; i < evenTs.size(); i++) {
		...
		auto t1 = evenTs[i];
		auto x1 =
			FrenetFrame{.o = cubicCurve(a.position, b, c, d, t1), .t = tangentCubicCurve(a.position, b, c, d, t1)};
...
FrenetFrame Bezier::frameAt(float t) const {
	...
	auto it = std::upper_bound(sortedFrames.begin(), sortedFrames.end(), t,
							   [](float t, const std::pair<float, FrenetFrame>& frame) { return t < frame.first; });
	...
	return FrenetFrame{.o = glm::mix(f0.o, f1.o, tInterp),
...
bool Bezier::recomputeIfDirty() {
	if (std::any_of(controlPoints.begin(), controlPoints.end(),
					[](std::shared_ptr<ControlPoint> const& p) { return p->dirty; })) {
		recompute();
		return true;
	}
	return false;
