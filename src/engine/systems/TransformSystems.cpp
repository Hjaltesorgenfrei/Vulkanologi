...
void RigidBodySystem::update(entt::registry &registry, float delta, entt::entity ent, PhysicsBody const &body,
							 Transform &transform) const {
	transform.modelMatrix = body.getTransform();
}

void TransformControlPointsSystem::update(entt::registry &registry, float delta, entt::entity ent,
										  Transform const &transform, ControlPointPtr &controlPoint) const {
	controlPoint.controlPoint->update(transform.modelMatrix);
}

void PhysicsCamera::update(entt::registry &registry, float delta, entt::entity ent, PhysicsBody const &body,
						   Camera &camera) const {
	...
	camera.camera.setCameraPosition(cameraPosition, delta);
	camera.camera.setTarget(cameraTarget, speed, delta);
}