...
void Renderer::createSwapChain() {
	...
	vk::SwapchainCreateInfoKHR createInfo{
		...
		.imageUsage = vk::ImageUsageFlagBits::eColorAttachment |
					  (vk::ImageUsageFlagBits::eTransferSrc & swapChainSupportedFlags)};
...
vk::SurfaceFormatKHR Renderer::chooseSwapSurfaceFormat(
	const std::vector<struct vk::SurfaceFormatKHR> &availableFormats) {
...
void Renderer::createImageViews() {
	...
	for (size_t i = 0; i < swapChainImages.size(); i++) {
		...
		deleteLaterQueue.push_function(
			[&, imageView = swapChainImageViews[i]]() { device->device().destroyImageView(imageView); });
	}
}
...
void Renderer::createRenderPass() {
	...
	vk::AttachmentDescription colorAttachmentResolve{.format = swapChainImageFormat,
	...
	const vk::RenderPassCreateInfo renderPassInfo{.attachmentCount = static_cast<uint32_t>(attachments.size()),
	...
	try {
		...
		mainDeletionQueue.push_function([&]() { device->device().destroyRenderPass(renderPass); });
	} catch (vk::SystemError &err) {
...
void Renderer::createPipelineLayout(vk::PipelineLayout &pipelineLayout,
	...
	try {
		...
		mainDeletionQueue.push_function([&]() { device->device().destroyPipelineLayout(pipelineLayout); });
	} catch (vk::SystemError &err) {
...
void Renderer::createLinePipeline() {
	...
	lineIndexBuffer =
		assetManager.allocatePersistentBuffer<uint32_t>(1000000 * 2, vk::BufferUsageFlagBits::eIndexBuffer);
...
void Renderer::createComputeShaderBuffers() {
	...
	auto usage = vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eVertexBuffer |
...
void Renderer::createFramebuffers() {
	...
	vk::FramebufferAttachmentImageInfo colorAttachResolve{
		.usage =
			vk::ImageUsageFlagBits::eColorAttachment | (vk::ImageUsageFlagBits::eTransferSrc & swapChainSupportedFlags),
		.width = swapChainExtent.width,
		.height = swapChainExtent.height,
		.layerCount = 1,
		.viewFormatCount = 1,
	...
	std::vector<vk::FramebufferAttachmentImageInfo> attachments = {colorAttach, depthAttach, colorAttachResolve};
	...
	try {
		...
		deleteLaterQueue.push_function(
			[&, cachedBuffer = swapChainFramebuffer]() { device->device().destroyFramebuffer(cachedBuffer); });
	} catch (vk::SystemError &err) {
...
void Renderer::createColorResources() {
	...
	colorImage = assetManager.createImage(
		swapChainExtent.width, swapChainExtent.height, 1, device->msaaSamples(), colorFormat, vk::ImageTiling::eOptimal,
		vk::ImageUsageFlagBits::eTransientAttachment | vk::ImageUsageFlagBits::eColorAttachment);
	colorImageView =
		createImageView(device->device(), colorImage._image, colorFormat, vk::ImageAspectFlagBits::eColor, 1);
...
void Renderer::createDepthResources() {
	...
	depthImage =
		assetManager.createImage(swapChainExtent.width, swapChainExtent.height, 1, device->msaaSamples(), depthFormat,
								 vk::ImageTiling::eOptimal, vk::ImageUsageFlagBits::eDepthStencilAttachment);
	depthImageView =
		createImageView(device->device(), depthImage._image, depthFormat, vk::ImageAspectFlagBits::eDepth, 1);
...
void Renderer::createDescriptorPool() {
	...
	mainDeletionQueue.push_function([&]() { device->device().destroyDescriptorPool(descriptorPool, nullptr); });
}
...
void Renderer::createDescriptorSets() {
	...
	for (size_t i = 0; i < swapChainImages.size(); i++) {
		vk::DescriptorBufferInfo bufferInfo{
			.buffer = uniformBuffers[i]->_buffer, .offset = 0, .range = sizeof(GlobalUbo)};
...
void Renderer::createComputeDescriptorSets() {
	...
	for (size_t i = 0; i < swapChainImages.size(); i++) {
		...
		vk::DescriptorBufferInfo thisFrameStorageBuffer{
			.buffer = shaderStorageBuffers[i]->_buffer, .offset = 0, .range = sizeof(Particle) * PARTICLE_COUNT};
...
void Renderer::recordCommandBuffer(vk::CommandBuffer &commandBuffer, size_t index, FrameInfo &frameInfo) {
	...
	vk::RenderPassBeginInfo renderPassInfo{.pNext = &attachBeginInfo,
										   .renderPass = renderPass,
	...
	commandBuffer.beginRenderPass(renderPassInfo, vk::SubpassContents::eInline);
	{
		...
		for (const auto &path : frameInfo.paths) {
			...
			commandBuffer.drawIndexed(static_cast<uint32_t>(path.getIndices().size()), 1, indexBackCount,
									  vertexBackCount, 0);
		}
	}
...
void Renderer::updateUniformBuffer(size_t currentImage, FrameInfo &frameInfo) {
	GlobalUbo ubo{.view = frameInfo.camera.viewMatrix(),
...
int Renderer::drawFrame(FrameInfo &frameInfo) {
	while (device->device().waitForFences(computeInFlightFences[currentFrame], VK_TRUE,
										  std::numeric_limits<uint64_t>::max()) == vk::Result::eTimeout) {
		// Wait
	}
	...
	while (device->device().waitForFences(inFlightFences[currentFrame], VK_TRUE,
										  std::numeric_limits<uint64_t>::max()) == vk::Result::eTimeout) {
		// Wait
	}
	...
	uint32_t imageIndex;
	try {
		const auto acquired = device->device().acquireNextImageKHR(swapChain, std::numeric_limits<uint64_t>::max(),
	...
	std::vector<vk::PipelineStageFlags> waitStages = {vk::PipelineStageFlagBits::eVertexInput,
