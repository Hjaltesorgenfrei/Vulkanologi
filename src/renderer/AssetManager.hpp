...
public:
	...
	[[nodiscard]] AllocatedImage createImage(int width, int height, uint32_t mipLevels,
											 vk::SampleCountFlagBits numSamples, vk::Format format,
											 vk::ImageTiling tiling, vk::ImageUsageFlags flags);

	template <typename T>
	[[nodiscard]] std::vector<std::shared_ptr<AllocatedBuffer>> createBuffers(std::span<T> data,
...
private:
	...
	[[nodiscard]] AllocatedImage createCubeImage(int width, int height, uint32_t mipLevels,
												 vk::SampleCountFlagBits numSamples, vk::Format format,
												 vk::ImageTiling tiling, vk::ImageUsageFlags flags);
...
inline AllocatedBuffer AssetManager::stageData(std::span<T>& dataToStage) {
	VkBufferCreateInfo stagingCreate{.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
	...
	if (vmaCreateBuffer(device->allocator(), &stagingCreate, &stagingAlloc,
						reinterpret_cast<VkBuffer*>(&stagingBuffer._buffer), &stagingBuffer._allocation,
						nullptr) != VK_SUCCESS) {
...
template <typename T>
inline std::vector<std::shared_ptr<AllocatedBuffer>> AssetManager::createBuffers(std::span<T> data,
																				 vk::BufferUsageFlags bufferUsage,
																				 size_t count) {
	...
	for (int i = 0; i < count; i++) {
		...
		if (vmaCreateBuffer(device->allocator(), &createInfo, &allocInfo, reinterpret_cast<VkBuffer*>(&buffer->_buffer),
							&buffer->_allocation, nullptr) != VK_SUCCESS) {
			throw std::runtime_error("Failed to create buffer!");
		}
		buffers.push_back(buffer);
		deletionQueue.push_function([buffer, this]() { cleanUpBuffer(*buffer); });
	}
